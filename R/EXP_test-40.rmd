...
```
Maggie between inverted only (inv), inverted and tail measured on desaturated (des) and inverted and tail measured on greyscale (gre):
...
```{r Rep,  warning = FALSE, message = FALSE, fig.width=4.75, fig.height=5.5}
  ...
  xy_ma[,comparison := 'within Mag inv, des, gre']
  xy_ma_id[,comparison := 'within Mag inv - des']
  xy_ma_ig[,comparison := 'within Mag inv - gre']
  xy_ma_dg[,comparison := 'within Mag des - gre']
  xy3 = rbind(xy_ma,xy_ma_id,xy_ma_ig,xy_ma_dg)

  xyy = rbind(xy_, xy2_)
  #xyy[, part := factor(part, levels=c("Acrosome", "Nucleus", "Midpiece","Tail", "Head", "Flagellum", "Total"))] 
  xyy[, part := factor(part, levels=c("Total", "Flagellum", "Head","Tail", "Midpiece", "Nucleus", "Acrosome"))] 

  g1 =
  ggplot(xyy, aes(x = part, y = pred, col = comparison, shape = method_CI)) +
  ...
  g2 =
  ggplot(xy3, aes(x = part, y = pred, col = comparison, shape = method_CI)) +
            geom_errorbar(aes(ymin = lwr, ymax = upr, col = comparison), width = 0.1, position = position_dodge(width = 0.9) ) +
            #ggtitle ("Sim based")+
            geom_point(position = position_dodge(width = 0.9)) +
            #scale_fill_brewer(palette = "Set1", guide = guide_legend(reverse = TRUE)) +
  ...
  ggL = ggarrange(legend1, legend2, heights=c(8, 2), nrow=2)
  ggarrange(ggA, ggL, ncol=2, widths=c(4, 2))
  #ggarrange(g1, g2, nrow=2, height=c(8, 2))
  #ggsave(file = here::here('Output/test-40_rep-ALL.jpeg'), ggarrange(ggA, ggL, ncol=2, widths=c(4, 2)), dpi = 300, width = 4.75, height = 5.5)
...
```{r R_M,  echo = FALSE, message = FALSE}

rbind(xy_ma,xy_ma_id,xy_ma_ig,xy_ma_dg)
