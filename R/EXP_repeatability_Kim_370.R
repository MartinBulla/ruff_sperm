...
# DATA
  d = data.table(read_excel('Data/370_remeasured.xlsx', sheet = 1))#, range = "A1:G161"))
  ...
  d[,ID := paste(Bird_ID,Photo_ID, Sperm_ID, Line_ID, Part)]

  w = data.table(read_excel('Data/370_remeasured.xlsx', sheet = 2))#, range = "A1:G161"))
  w[, February:=as.character(February) ]
  stri_sub(w$February,nchar(w$February)-2, nchar(w$February)-3) = "."
  w[,February :=as.numeric(February) ]
  w[,April :=as.character(April) ]
  stri_sub(w$April,nchar(w$April)-2, nchar(w$April)-3) = "."
  w[,April :=as.numeric(April) ]
  w[Part == 'Mid piece', Part := 'Midpiece']
  w[Part == 'Flagellum', Part := 'Tail']
  w[Part == 'Head', Part := 'Nucleus']
# plot correlations
  g1 = ggplot(w[!Part%in%c('Head','Flagellum','Total')], aes(x = February, y = April)) +
  ...
  #ggtitle('All')+
  theme_bw()
  ggsave('Output/VD_370_Kim_correlation_BASIC.png',g1, width = 10, height =10, units = 'cm')
...
# plot repeatability
  ...
  # plot BASIC
    ...
    ggsave('Output/VD_370_Kim_Repeatability_BASIC.png',g1, width = 10, height =7, units = 'cm')
